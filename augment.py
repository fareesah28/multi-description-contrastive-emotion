...
OUTPUT_CSV   = "/mnt/scratch/users/adhg808/MAFW/Labels/generated_captions.csv"
NUM_FRAMES   = 8
NUM_CAPTIONS = 5
...
# load human-written caption from csv
df_human = pd.read_excel(LABELS_XLSX, header=None, engine='openpyxl')
human_map = {str(row[0]): str(row[2]) for _, row in df_human.iterrows()}
...
for video_file in video_list:
    ...
    # get human caption (or skip if not found)
    human_caption = human_map.get(video_file, "")
    if human_caption.strip() == "":
        print(f"Skipping {video_file} — no matching human caption.")
        continue
    ...
    for i in range(NUM_CAPTIONS):
        ...
        print(f"🔹 Caption {i+1}: {caption}")  # log to stdout
        captions.append(caption)

    # append result row
    results.append([video_file, human_caption] + captions)

# save all results to csv
df_out = pd.DataFrame(results, columns=[
    "video", "human_caption",
    "llava_caption_1", "llava_caption_2", "llava_caption_3", "llava_caption_4", "llava_caption_5"
])
